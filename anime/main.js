...
async function loadMangaChapters(){
  ...
  let res= await fetch(mangaUrl+'info/'+currentId+'?provider=mangadex')
  let data=await res.json()

   elem('#manga-screen').innerHTML=''
   elem('#chapter-list').innerHTML=''

    data.chapters.forEach((chapter,index)=>{
      let chapterDiv=document.createElement('option')      
      chapterDiv.innerHTML=`chapter ${chapter.chapterNumber} - ${chapter.title}`
      chapterDiv.value=chapter.id
      

      elem('#chapter-list').appendChild(chapterDiv)
    })
    let chapterNo=new URLSearchParams(location.search).get('chapter')
    elem('#chapter-list').value=data.chapters[chapterNo].id
    
  
   
    let resM=await fetch(mangaUrl+'read?chapterId='+elem('#chapter-list').value+'&provider=mangadex')
     let dataM=await resM.json()
     elem('#manga-screen').innerHTML=''
        dataM.forEach(page=>{
        let managapage=document.createElement('img')
        managapage.classList.add('img','manga-img')
        managapage.setAttribute('src',page.img)

        elem('#manga-screen').appendChild(managapage)
        })
...
elem('#details-blured-img').setAttribute('src',data.image)
  ...
  if(data.type=='MANGA'){
    ...
    elem('#watch-btn').onclick=()=>{
      ...
      routeButReplace('/manga?id='+data.id+'&chapter='+1)
...
elem('#nextChapBtn').addEventListener('click',()=>{
let currChap=parseInt(new URLSearchParams(location.search).get('chapter'))
currChap+=1
let currMangaId=new URLSearchParams(location.search).get('id')

routeButReplace(`/manga?id=${currMangaId}&chapter=${currChap}`)


})
elem('#prevChapBtn').addEventListener('click',()=>{
  let currChap=parseInt(new URLSearchParams(location.search).get('chapter'))
  currChap-=1
  let currMangaId=new URLSearchParams(location.search).get('id')
  ...
  elem('#chapter-list').addEventListener('change',()=>{
    let currMangaId=new URLSearchParams(location.search).get('id')
    routeButReplace(`/manga?id=${currMangaId}&chapter=${elem('#chapter-list').selectedIndex}`)
  })
